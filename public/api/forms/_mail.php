...
function send_email_via_smtp(array $smtp, array $message): array
{
    ...
    if ($secure === 'tls') {
        ...
        if (!$ehlo2['ok']) {
            fclose($fp);
            return ['ok' => false, 'error' => 'SMTP EHLO after TLS failed'];
        }
    }
    ...
    if (!$userResp['ok']) {
        fclose($fp);
        return ['ok' => false, 'error' => 'SMTP AUTH username rejected'];
    }

    $passResp = smtp_command($fp, base64_encode($password), [235]);
    if (!$passResp['ok']) {
        fclose($fp);
        return ['ok' => false, 'error' => 'SMTP AUTH password rejected'];
    }
    ...
    if (!$rcptTo['ok']) {
        fclose($fp);
        return ['ok' => false, 'error' => 'SMTP RCPT TO failed'];
    }

    $data = smtp_command($fp, 'DATA', [354]);
    if (!$data['ok']) {
        fclose($fp);
        return ['ok' => false, 'error' => 'SMTP DATA failed'];
    }
    ...
    if (!smtp_is_ok($dataDone, [250])) {
        fclose($fp);
        return ['ok' => false, 'error' => 'SMTP message rejected'];
    }
